...
def get_time_delta(time_string: str) -> timedelta:
    ...
    parts: Optional[Match[str]] = rel_time.match(string=time_string)
    if not parts:
        raise DagFactoryException(f"Invalid relative time: {time_string}")
    # https://docs.python.org/3/library/re.html#re.Match.groupdict
    parts_dict: Dict[str, str] = parts.groupdict()
    time_params = {}
    if all(value is None for value in parts_dict.values()):
        raise DagFactoryException(f"Invalid relative time: {time_string}")
    for time_unit, magnitude in parts_dict.items():
        if magnitude:
...
def get_python_callable(python_callable_name: str, python_callable_file: str) -> Callable:
    """
    ...
    spec = importlib.util.spec_from_file_location(module_name, python_callable_file)  # type: ignore
...
def get_python_callable_lambda(lambda_expr: str) -> Callable:
    """
