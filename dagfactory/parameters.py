...
* :mod:`dagfactory.dagbuilder` uses it to decide which keys reach the ``DAG``
  ...
  aliases.
* :mod:`dagfactory.validator` uses it to report the same facts as ``dagfactory
  lint`` diagnostics.

Stating a fact once is the point: a version bound written here changes what the
builder forwards and what the linter reports, together.
