...
from kubernetes.client.models import (V1EnvVar, V1EnvVarSource,
                                      V1ObjectFieldSelector, V1Pod, V1Volume)
...
class DagBuilder:
    ...
    def get_dag_params(self) -> Dict[str, Any]:
        ...
        if utils.check_dict_key(dag_params["default_args"], "on_success_callback"):
            if isinstance(dag_params["default_args"]["on_success_callback"], str):
                dag_params["default_args"]["on_success_callback"] = import_string(
        ...
        if utils.check_dict_key(dag_params["default_args"], "on_failure_callback"):
            if isinstance(dag_params["default_args"]["on_failure_callback"], str):
                dag_params["default_args"]["on_failure_callback"] = import_string(
        ...
        if utils.check_dict_key(dag_params["default_args"], "on_retry_callback"):
            if isinstance(dag_params["default_args"]["on_retry_callback"], str):
                dag_params["default_args"]["on_retry_callback"] = import_string(
        ...
        if utils.check_dict_key(dag_params, "on_failure_callback_name") and utils.check_dict_key(
            ...
            dag_params["on_failure_callback"] = utils.get_python_callable(
        ...
        if utils.check_dict_key(dag_params, "template_searchpath"):
            if isinstance(dag_params["template_searchpath"], (list, str)) and utils.check_template_searchpath(
                ...
                dag_params["template_searchpath"] = dag_params["template_searchpath"]
            else:
        ...
        if utils.check_dict_key(dag_params, "render_template_as_native_obj"):
            if isinstance(dag_params["render_template_as_native_obj"], bool):
                dag_params["render_template_as_native_obj"] = dag_params["render_template_as_native_obj"]
            else:
    ...
    @staticmethod
    def make_task(operator: str, task_params: Dict[str, Any]) -> Union[BaseOperator, MappedOperator]:
        """
        ...
        try:
            if issubclass(operator_obj, (PythonOperator, BranchPythonOperator, PythonSensor)):  # type: ignore
                if (
                ...
                if not task_params.get("python_callable"):
                    task_params["python_callable"] = utils.get_python_callable(
            ...
            if issubclass(operator_obj, SqlSensor):  # type: ignore
                # Success checks
                if task_params.get("success_check_file") and task_params.get("success_check_name"):
                    task_params["success"] = utils.get_python_callable(
                    ...
                    task_params["success"] = utils.get_python_callable_lambda(task_params["success_check_lambda"])
                    del task_params["success_check_lambda"]
                # Failure checks
                if task_params.get("failure_check_file") and task_params.get("failure_check_name"):
                    task_params["failure"] = utils.get_python_callable(
                    ...
                    task_params["failure"] = utils.get_python_callable_lambda(task_params["failure_check_lambda"])
                    del task_params["failure_check_lambda"]

            if issubclass(operator_obj, HttpSensor):  # type: ignore
                if not (
                ...
                if task_params.get("response_check_file"):
                    task_params["response_check"] = utils.get_python_callable(
                    ...
                    task_params["response_check"] = utils.get_python_callable_lambda(
            ...
            # KubernetesPodOperator
            if issubclass(operator_obj, KubernetesPodOperator):  # type: ignore
                task_params["secrets"] = (
            ...
            if utils.check_dict_key(task_params, "execution_timeout_secs"):
                task_params["execution_timeout"] = timedelta(seconds=task_params["execution_timeout_secs"])
                del task_params["execution_timeout_secs"]

            if utils.check_dict_key(task_params, "sla_secs"):
                task_params["sla"] = timedelta(seconds=task_params["sla_secs"])
                del task_params["sla_secs"]

            if utils.check_dict_key(task_params, "execution_delta_secs"):
                task_params["execution_delta"] = timedelta(seconds=task_params["execution_delta_secs"])
            ...
            if utils.check_dict_key(task_params, "execution_date_fn_name") and utils.check_dict_key(
                ...
                task_params["execution_date_fn"] = utils.get_python_callable(
            ...
            if utils.check_dict_key(task_params, "on_execute_callback") and version.parse(
                ...
                task_params["on_execute_callback"] = import_string(task_params["on_execute_callback"])

            if utils.check_dict_key(task_params, "on_failure_callback"):
                task_params["on_failure_callback"] = import_string(task_params["on_failure_callback"])

            if utils.check_dict_key(task_params, "on_success_callback"):
                task_params["on_success_callback"] = import_string(task_params["on_success_callback"])

            if utils.check_dict_key(task_params, "on_retry_callback"):
                task_params["on_retry_callback"] = import_string(task_params["on_retry_callback"])

            # use variables as arguments on operator
            if utils.check_dict_key(task_params, "variables_as_arguments"):
                variables: List[Dict[str, str]] = task_params.get("variables_as_arguments")  # type: ignore
            ...
            task: Union[BaseOperator, MappedOperator] = (
                ...
                else operator_obj.partial(**task_params).expand(**expand_kwargs)  # type: ignore
            )
        except Exception as err:
    ...
    def make_task_groups(task_groups: Dict[str, Any], dag: DAG) -> Dict[str, "TaskGroup"]:
        ...
        if version.parse(AIRFLOW_VERSION) >= version.parse("2.0.0"):
            for task_group_name, task_group_conf in task_groups.items():
                ...
                task_groups_dict[task_group.group_id] = task_group  # type: ignore
    ...
    def set_dependencies(
        ...
        for name, conf in tasks_and_task_groups_config.items():
            ...
            if conf.get("dependencies"):
                source: Union[BaseOperator, "TaskGroup"] = tasks_and_task_groups_instances[name]  # type: ignore
    ...
    def build(self) -> Dict[str, Union[str, DAG]]:
        ...
        if version.parse(AIRFLOW_VERSION) >= version.parse("2.2.0"):
            ...
            if dag_params.get("timetable"):
                timetable_args = dag_params["timetable"]
        ...
        if dag_params.get("doc_md_file_path"):
            ...
            with open(dag_params.get("doc_md_file_path"), "r", encoding="utf-8") as file:  # type: ignore
        ...
        if dag_params.get("doc_md_python_callable_file") and dag_params.get("doc_md_python_callable_name"):
            doc_md_callable = utils.get_python_callable(
                dag_params.get("doc_md_python_callable_name"),  # type: ignore
                dag_params.get("doc_md_python_callable_file"),  # type: ignore
            )
        ...
        dag.is_dagfactory_auto_generated = True  # type: ignore
        ...
        for task_name, task_conf in tasks.items():
            ...
            if task_groups_dict and task_conf.get("task_group_name"):
                task_conf["task_group"] = task_groups_dict[task_conf.get("task_group_name")]  # type: ignore
            ...
            tasks_dict[task.task_id] = task  # type: ignore
