...
def test_resolve_taskflow_value_plus_prefix_with_xcom_key_subscript():
    # multiple_outputs=True case: +task["xcom_key"], matching dagbuilder's _replace_kwargs_values_as_xcom
    result = resolve_taskflow_value("value", "+collect['key1']", set())
    assert result == "collect['key1']"


def test_resolve_taskflow_value_plus_prefix_with_xcom_key_double_quoted():
    result = resolve_taskflow_value("value", '+collect["key2"]', set())
    assert result == "collect['key2']"
